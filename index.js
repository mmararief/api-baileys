...
const generateInvoicePDF = async (data) => {
    const filePath = path.join(__dirname, 'invoices', `invoice_${data.id_transaksi}_${data.nama}.pdf`);
    ...
    // Add logo and content to PDF
    const logoPath = path.join(__dirname, 'img', 'logo_kynan.png');
    ...
    doc.fontSize(12)
       .fillColor('black')
       .text(`ID Faktur: ${data.id_transaksi}`, { align: 'center' })
       .moveDown(0.5)
